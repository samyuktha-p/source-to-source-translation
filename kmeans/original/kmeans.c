...
int allocate(int npoints, int nfeatures, int nclusters, float **feature, float* temp_feature_swap)
{
	...
	//write buffers
	err = clEnqueueWriteBuffer(cmd_queue, d_feature, 1, 0, npoints * nfeatures * sizeof(float), feature[0], 0, 0, 0);
	...
	err = clEnqueueReadBuffer(cmd_queue, d_feature_swap, 1, 0, npoints * nfeatures * sizeof(float), temp_feature_swap, 0, 0, 0);
	if(err != CL_SUCCESS) { printf("ERROR: clEnqueueNDRangeKernel()=>%d failed\n", err); return -1; }
           ...
           float  **new_centers)	
{
	...
	err = clEnqueueWriteBuffer(cmd_queue, d_feature_swap, 1, 0, nclusters * nfeatures * sizeof(float), temp_feature_swap, 0, 0, 0);
	if(err != CL_SUCCESS) { printf("ERROR: clEnqueueWriteBuffer d_cluster (size:%d) => %d\n", npoints, err); return -1; }
