...
__host __dma_aligned long long feature_swap_map[1];
__host __dma_aligned long long feature_swap_stride[1];
__host __dma_aligned long long feature_swap_length[1];
__host __dma_aligned long long feature_swap_flag[1];
__host __dma_aligned long long feature_swap_cpu_offset[1];
__host __dma_aligned long long feature_swap_offset[1];
...
__host __dma_aligned __mram_ptr float * feature_swap;
...
__host __dma_aligned int64_t feature_swap_start = SIZE_DEFAULT;

void kmeans_swap(int INPUT, int TASKLET_ID_WG, int * t_bd, Point * t_td_se)
{
int ss_tid = me();
int ss_tx = 0, ss_ty = 0, ss_tz = 0;
unsigned int (* tid)[((t_td_se[0].end)-(t_td_se[0].start))] = mem_alloc(((t_td_se[0].end)-(t_td_se[0].start))*sizeof (unsigned int));
unsigned int (* tid_relative)[((t_td_se[0].end)-(t_td_se[0].start))] = mem_alloc((((t_td_se[0].end)-(t_td_se[0].start))*sizeof (unsigned int)));;
if(ss_tid == 0) {
feature = DPU_MRAM_HEAP_POINTER + feature_start; 
feature_swap = DPU_MRAM_HEAP_POINTER + feature_swap_start; 
}
if(TASKLET_ID_WG == 0) {
}
...
{
( * tid)[ss_tx-(t_td_se[0].start)]=get_global_id(0, t_bd[0], ss_tx);
( * tid_relative)[ss_tx-(t_td_se[0].start)]=get_relative_global_id(0, t_bd[0], ss_tx);
}
/* for(int i = 0; i <  nfeatures; i++) */
/* 	feature_swap[i npoints + tid] = feature[tid * nfeatures + i]; */
/* Lingjie Zhang modificated at 1105/2015 */
for (ss_tx=(t_td_se[0].start); ss_tx<(t_td_se[0].end); ss_tx ++ )
{
if (( * tid)[ss_tx-(t_td_se[0].start)]<npoints)
{
{
int i = 0;
#pragma loop name kmeans_swap#0 
for (; i<nfeatures; i ++ )
{
feature_swap[(i*npoints)+( * tid_relative)[ss_tx-(t_td_se[0].start)]]=feature[(( * tid_relative)[ss_tx-(t_td_se[0].start)]*nfeatures)+i];
}
}
}
}
...
kmeans_swap(0, TASKLET_ID_WG, t_bd, t_td_se);
