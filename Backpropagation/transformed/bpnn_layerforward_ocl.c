...
__host __dma_aligned long long hidden_partial_sum_map[1];
__host __dma_aligned long long hidden_partial_sum_stride[1];
__host __dma_aligned long long hidden_partial_sum_length[1];
__host __dma_aligned long long hidden_partial_sum_flag[1];
__host __dma_aligned long long hidden_partial_sum_cpu_offset[1];
__host __dma_aligned long long hidden_partial_sum_offset[1];
__host __dma_aligned long long input_hidden_cuda_map[1];
__host __dma_aligned long long input_hidden_cuda_stride[1];
__host __dma_aligned long long input_hidden_cuda_length[1];
__host __dma_aligned long long input_hidden_cuda_flag[1];
__host __dma_aligned long long input_hidden_cuda_cpu_offset[1];
__host __dma_aligned long long input_hidden_cuda_offset[1];
__host __dma_aligned long long input_cuda_map[1];
__host __dma_aligned long long input_cuda_stride[1];
__host __dma_aligned long long input_cuda_length[1];
__host __dma_aligned long long input_cuda_flag[1];
__host __dma_aligned long long input_cuda_cpu_offset[1];
__host __dma_aligned long long input_cuda_offset[1];
int max(int a, int b)
{
	return (a<b) ?b :a;
}
...
__host __dma_aligned __mram_ptr float * input_cuda;
__host __dma_aligned __mram_ptr float * output_hidden_cuda;
__host __dma_aligned __mram_ptr float * input_hidden_cuda;
__host __dma_aligned __mram_ptr float * hidden_partial_sum;
float * input_node[N_MULTI_WGS];
float * weight_matrix[N_MULTI_WGS];
__host __dma_aligned int in;
__host __dma_aligned int hid;
#define SIZE_DEFAULT 100

__host __dma_aligned int64_t input_cuda_start = SIZE_DEFAULT;
__host __dma_aligned int64_t output_hidden_cuda_start = SIZE_DEFAULT;
__host __dma_aligned int64_t input_hidden_cuda_start = SIZE_DEFAULT;
__host __dma_aligned int64_t hidden_partial_sum_start = SIZE_DEFAULT;
__host __dma_aligned int64_t input_node_size = SIZE_DEFAULT;
__host __dma_aligned int64_t weight_matrix_size = SIZE_DEFAULT;

void bpnn_layerforward_ocl(int INPUT, int TASKLET_ID_WG, int * t_bd, Point * t_td_se)
{
int tid = me();
...
if(tid == 0) {
input_cuda = DPU_MRAM_HEAP_POINTER + input_cuda_start; 
output_hidden_cuda = DPU_MRAM_HEAP_POINTER + output_hidden_cuda_start; 
input_hidden_cuda = DPU_MRAM_HEAP_POINTER + input_hidden_cuda_start; 
hidden_partial_sum = DPU_MRAM_HEAP_POINTER + hidden_partial_sum_start; 
}
if(TASKLET_ID_WG == 0) {
input_node[t_bd[PARTITION_DIM_GRID]] = mem_alloc(input_node_size);
weight_matrix[t_bd[PARTITION_DIM_GRID]] = mem_alloc(weight_matrix_size);
}
...
{
input_node[t_bd[PARTITION_DIM_GRID]][ty]=input_cuda[((16*by)+ty)+0];
}
}
}
barrier_wait(barrier[t_bd[PARTITION_DIM_GRID]]);
...
for (tx=(t_td_se[0].start); tx<(t_td_se[0].end); tx ++ )
{
weight_matrix[t_bd[PARTITION_DIM_GRID]][(ty*16)+tx]=input_hidden_cuda[((((((hid+1)*16)*by)+((hid+1)*ty))+tx)+0)+0];
}
}
barrier_wait(barrier[t_bd[PARTITION_DIM_GRID]]);
//++barrier_count[tid];
...
for (tx=(t_td_se[0].start); tx<(t_td_se[0].end); tx ++ )
{
weight_matrix[t_bd[PARTITION_DIM_GRID]][(ty*16)+tx]=(weight_matrix[t_bd[PARTITION_DIM_GRID]][(ty*16)+tx]*input_node[t_bd[PARTITION_DIM_GRID]][ty]);
}
}
barrier_wait(barrier[t_bd[PARTITION_DIM_GRID]]);
//++barrier_count[tid];
...
{
int i = 1;
#pragma cetus private(power_two) 
#pragma loop name bpnn_layerforward_ocl#0 
for (; i<=16; i=(i*2))
{
/* for ( int i = 1 ; i <= 4 ; i++){ */
int power_two = i;
...
for (tx=(t_td_se[0].start); tx<(t_td_se[0].end); tx ++ )
{
weight_matrix[t_bd[PARTITION_DIM_GRID]][(ty*16)+tx]=(weight_matrix[t_bd[PARTITION_DIM_GRID]][(ty*16)+tx]+weight_matrix[t_bd[PARTITION_DIM_GRID]][((ty+(power_two/2))*16)+tx]);
}
}
}
barrier_wait(barrier[t_bd[PARTITION_DIM_GRID]]);
...
for (tx=(t_td_se[0].start); tx<(t_td_se[0].end); tx ++ )
{
input_hidden_cuda[((((((hid+1)*16)*by)+((hid+1)*ty))+tx)+0)+0]=weight_matrix[t_bd[PARTITION_DIM_GRID]][(ty*16)+tx];
}
}
barrier_wait(barrier[t_bd[PARTITION_DIM_GRID]]);
//++barrier_count[tid];
...
for (ty=(t_td_se[1].start); ty<(t_td_se[1].end); ty ++ )
{
hidden_partial_sum[(by*hidden_partial_sum_stride[0])+ty]=weight_matrix[t_bd[PARTITION_DIM_GRID]][(tx*16)+ty];
}
}
}
return ;
...
bpnn_layerforward_ocl(0, TASKLET_ID_WG, t_bd, t_td_se);
