...
int bpnn_train_kernel(BPNN *net, float *eo, float *eh)
{
  ...
  //newly added - samyuktha
	err = clEnqueueReadBuffer(cmd_queue, input_prev_weights_ocl, 1, 0, (in + 1) * (hid + 1) * sizeof(float), input_weights_prev_one_dim, 0, 0, 0);
	if(err != CL_SUCCESS) { printf("ERROR: 1  clEnqueueReadBuffer: input_prev_weights_ocl\n"); return -1; }
