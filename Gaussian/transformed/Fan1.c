...
__host __dma_aligned long long m_dev_map[1];
__host __dma_aligned long long m_dev_stride[1];
__host __dma_aligned long long m_dev_length[1];
__host __dma_aligned long long m_dev_flag[1];
__host __dma_aligned long long m_dev_cpu_offset[1];
__host __dma_aligned long long m_dev_offset[1];
int max(int a, int b)
{
	return (a<b) ?b :a;
}
...
void Fan1(int INPUT, int TASKLET_ID_WG, int * t_bd, Point * t_td_se)
{
int tid = me();
...
for (ss_tx=(t_td_se[0].start); ss_tx<(t_td_se[0].end); ss_tx ++ )
{
if (( * globalId)[ss_tx-(t_td_se[0].start)]<((size-1)-t))
{
/* (m_dev + size * (globalId + t + 1)+t) = *(a_dev + size * (globalId + t + 1) + t) / *(a_dev + size * t + t);     */
m_dev[(m_dev_stride[0]*((( * globalId_relative)[ss_tx-(t_td_se[0].start)]+0)+0))+0]=(a_dev[(a_dev_stride[0]*((( * globalId_relative)[ss_tx-(t_td_se[0].start)]+0)+0))+0]/a_dev[0+a_dev_offset[1]]);
}
}
return ;
}
...
Fan1(0, TASKLET_ID_WG, t_bd, t_td_se);
